...
try:
    ...
    while not rospy.is_shutdown() and flag==0:
        if message_arrived:
            ...
            #x_left=[500]*len(time)
            #pitch candy:rain
            images_candy_small=[2]*8
            images_pero=[16]*5
            images_candy=[1]*(len(time)-13)
            images=np.concatenate([images_candy_small,images_candy,images_pero],0)
            ...
            while True:
                ...
                if len(info)>0 and len(np.where((um_pos_y-50<info[:,2]) & (info[:,2]<um_pos_y+50))[0])>0:
                    ...
                    for m in range(len(num_f)):
                        ...
                        elif um_pos_x+140<=keep[1] and keep[1]<um_pos_x+280 and keep[0]!=2:
                            #x_centerof parabola
                        ...
                        elif um_pos_x-420<=keep[1] and keep[1]<um_pos_x-280 and keep[0]!=2:
                            #x_centerof parabola
                            keep.append(keep[1]-110)
                            #remove
                            info[num_f[m]][4]=0
                ...
                if len(flip_info_rfar_s)>0:
                    flip_info_rfar_s[:,1]+=np.array([15]*len(flip_info_rfar),dtype=np.int)
                    #make parabola
